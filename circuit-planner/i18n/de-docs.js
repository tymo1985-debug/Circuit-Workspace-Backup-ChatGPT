/**
 * Клиндарий — немецкий документный слой.
 *
 * Step 29: формуляр визита, немецкие PDF и письма вынесены из общего runtime
 * в самостоятельный модуль. Он не меняет App.config.version и не мигрирует
 * пользовательские данные. JW-терминология сохранена из ранее сверенного слоя.
 */
(function () {
  'use strict';

  if (window.CWKlindariyGermanDocumentsLoaded) return;
  window.CWKlindariyGermanDocumentsLoaded = true;
...
<div>• Änderungen oder besondere Bedürfnisse bitte möglichst früh mitteilen.</div>`
  ...
  if (typeof window.CWKlindariyRegisterPreInitHook !== 'function') {
    console.error('circuit-planner/i18n/de-docs.js: pre-init hook API недоступен');
    return;
  }

  window.CWKlindariyRegisterPreInitHook(function installGermanDocuments(app) {
    if (app && app.ui) {
