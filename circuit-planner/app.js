...
• У вівторок ввечері перед зустріччю я хотів би зустрітися з одним із старійшин збору, з координатором ради старійшин, чи з іншим старійшиною. Ми можемо домовитися про час і місце зустрічі перед початком візиту.`;
  ...
  const App = {
    ...
    ui: {
      ...
      renderNav() {
        const buildButton = (item, mobile = false) => `<button class="${mobile ? 'bottom-nav-btn' : 'nav-btn md-sidenav__item md-state-layer'} ${App.state.selectedScreen === item.id ? 'active' : ''}" data-screen="${item.id}" type="button"><span class="icon">${item.icon}</span><span class="label">${App.utils.t(item.tKey)}</span></button>`;
