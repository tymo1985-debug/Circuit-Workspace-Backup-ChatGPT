/**
 * Клиндарий — ранний bootstrap + регистрация PWA.
 *
 * Этот файл выполняется с `defer` непосредственно ПЕРЕД app.js. Поэтому здесь
 * находится маленький pre-init мост для немецкого интерфейса: он успевает
 * зарегистрировать словарь и расширить App.i18nBridge до первого App.init().
 *
 * ВАЖНО: язык документов сюда не переносится. Письмо и формуляр визита по-прежнему
 * управляются своим языком документа через shared/doclang.js.
 */
(function () {
  'use strict';
